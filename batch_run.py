...
results_filtered[["iteration","Step", "Max Flood Inundation", "Migration Count",'Adaptation','people_per_household']].reset_index(drop=True)
results_filtered["Step"] += 2010
# results_filtered["iteration"] += 1

# aggregate household preparedness

agg_results_df = (results_filtered.groupby(["iteration","people_per_household","Step"]).agg({"Adaptation":"mean"}).reset_index())
...
for i in range(2):  
    sns.lineplot(data=results_filtered, ax=axes[i], x="Step", y=ys[i], hue="people_per_household", palette="dark:#5A9_r")
    axes[i].set(
        xlabel="Year",
        ylabel=ylabels[i],
        title=titles[i],
...
sns.lineplot(data=results_filtered, ax=axes, x="Step", y=ys[2], hue="people_per_household", palette="dark:#5A9_r")
axes.set(
    xlabel="Year",
    ylabel=ylabels[2],
    title=titles[2]
